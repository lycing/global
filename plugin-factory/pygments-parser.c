...
static int 
get_script_path(char script_path[]) {
  if (access(PYGMENTS_PARSER, R_OK) == 0) {
    return sprintf(script_path, "%s", PYGMENTS_PARSER);
  }
  return sprintf(script_path, "%s/gtags/share/gtags/script/pygments_parser.py", getenv("HOME"));
}
...
start_process(const struct parser_param *param)
{
  ...
  char *argv[] = {
	...
	if (pid == 0) {
		...
		param->die("cannot execute '%s'. (execvp failed)", script_path);
	}
	/* parent process */
	if (pid < 0)
